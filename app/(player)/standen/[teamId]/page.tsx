...
export default function TeamDetailPage({ params }: { params: Promise<{ teamId: string }> }) {
  ...
  const toggleFav = () => {
    const next = isFav ? null : teamId
    try { if (next) localStorage.setItem('vb_fav', next); else localStorage.removeItem('vb_fav') } catch { /* ignore */ }
    setFav(next)
  }
      ...
      <MobileHeader
        ...
        right={
          <button
            ...
            style={{
              border: 'none',
              background: isFav ? 'var(--primary)' : 'var(--surface-2)',
              width: 42,
              height: 42,
              borderRadius: 999,
              cursor: 'pointer',
          ...
          >
            <Icon name="ster" size={20} color={isFav ? 'var(--primary-ink)' : 'var(--ink-3)'} filled={isFav} />
          </button>
        }
      />
